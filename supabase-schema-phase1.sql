...
BEGIN
  ...
  IF length(v_username) < 3 OR length(v_username) > 24 THEN RAISE EXCEPTION 'Invalid length'; END IF;
  IF v_username !~ '^[a-z0-9_]+(search_username TEXT) 
...
GRANT EXECUTE ON FUNCTION create_profile(TEXT, TEXT) TO authenticated;
 THEN RAISE EXCEPTION 'Invalid format'; END IF;
  ...
  VALUES (v_uid, v_username, p_display_name, (extract(epoch from now()) * 1000)::bigint);
END;
$func$;
